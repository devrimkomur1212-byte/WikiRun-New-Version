...
async function resolveMatch(matchId: string) {
  ...
  // Early exit if already resolved (prevents double resolution from race condition)
  if ((matchData as MatchRow).status === "complete") {
    console.log("Match already resolved, skipping");
    return;
  }
  ...
  // This prevents double resolution when both players' calls race
